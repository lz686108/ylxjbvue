  ...
  <div>
    ...
    <div class="fh">
      ...
      <div class="ery">
        ...
        <div class="qhbb2" style="border-radius: 0.2vw">
          <div class="qhbb4" style="font-size: 0.8vw" @click="qergtmi()">
            理化性能
          </div>
        </div>

        <div class="qhbb2" style="border-radius: 0.2vw">
          <div class="qhbb4" style="font-size: 0.8vw" @click="insertymbbh()">
            保存
          </div>
        </div>
        <div class="qhbb2" style="border-radius: 0.2vw">
    ...
    <div class="thres">
      <div>
        ...
        <select
          ...
          v-model="gysmc"
        >
          <option v-for="(item, index) in selectbuts.ymmc" :key="index">
            {{ item }}
          </option>
        </select>
      </div>
      ...
      <div>
        <span class="texts" style="margin-left: 1.5vw"> 运费价格: </span>
    ...
    <div class="tablebackground">
      <table class="table">
        ...
        <tr v-for="(item, index) in tablelists" :key="index">
          ...
          <td style="width: 23vw;text-align: left">{{ item.distributorName }}</td>
    ...
    <div class="xiugai" v-if="updateshow == true">
      <div class="xiugaiyemian">
        ...
        <div class="tfh">
          <div>
            <span class="texts"> 月计划量 </span>
            <input type="text" class="inputs" v-model="yjhl"/>
          </div>
          <div>
            <span class="texts" style="margin-left: 1.9vw"> 付款方式 </span>
            <input type="text" class="inputs" v-model="fkfs"/>
          </div>
        </div>
        <!-- 第三行 -->
        <div class="tfh">
          <div>
            <span class="texts"> 结算水分 </span>
            <input type="text" class="inputs" v-model="jssf"/>
          </div>
          <div>
            <span class="texts" style="margin-left: 1.9vw"> 运费价格 </span>
            <input type="text" class="inputs" v-model="jsfm"/>
          </div>
        </div>
        <!-- 第五行按钮行 -->
...
export default {
  ...
  methods: {
    qergtmi() {
      this.$router.push({path: "/lhxnym", query: {}})
    },
    //烟煤新增版本
    insertymbbh(){
        ...
        axios({
          method: "post",
          url: "http://10.6.1.179:40018/mpd/ylxjb/luruf/insertymversions",
          params: {
    ...
    selectattributevalue() {
      axios({
        ...
        this.gysmc = this.selectbuts.ymmc[res.data.ymmc.length - 1];
      });
    },
    //信息添加至表格中
    insertallofluruf() {
      ...
      axios({
        method: "post",
        url: "http://10.6.1.179:40018/mpd/ylxjb/luruf/insertymdcwh",
        params: {
    ...
    selectofluruf() {
      axios({
        method: "post",
        url: "http://10.6.1.179:40018/mpd/ylxjb/luruf/selectymdccx",
        params: {},
      }).then((res) => {
        this.tablelists = res.data;
      });
    ...
    //修改信息
    updatecontext(
      distributorName, monthAmount, contractPrice, jsfm, sterilise, jssf, id) {
    ...
    quedingupdate() {
      ...
      axios({
        method: "post",
        url: "http://10.6.1.179:40018/mpd/ylxjb/luruf/updateymwhxg",
        params: {
          gysmc: this.gysmc,
          fkfs: this.fkfs,
          htjg: this.htjg,
        ...
        //修改之后查询表格信息
        this.updateshow = false;
        this.gysmc = this.selectbuts.ymmc[this.selectbuts.ymmc.length - 1];
        this.htjg = "";
        this.yjhl = "";
        this.$message("修改成功");
      });
    ...
    fhtursec() {
      this.$router.push({path: "/ymdc", query: {}});
    },
    xpwhtur() {
    ...
    htjgtjtj() {
      ...
      if (this.htjgzj != "") {
        axios({
          method: "post",
          url: "http://10.6.1.179:40018/mpd/ylxjb/luruf/updateymdcwhprice",
          params: {
      ...
      if (this.htjgjs != "") {
        axios({
          method: "post",
          url: "http://10.6.1.179:40018/mpd/ylxjb/luruf/updatetjym",
          params: {
            tabli: JSON.stringify(this.tablelists),
            htjgjs: this.htjgjs,
          },
    ...
    bzwhxxy() {
      //标准维护弹窗
      this.$router.push({path: "/ymfw", query: {}})
    },
    // 标准信息查询
    bzxxcx() {
      axios({
    ...
    //基本表维护页面
    jcwhbut() {
      this.$router.push({path: "/ymjc", query: {}});
    },
    /**
     * 删除供应商
     */
    deletegysmc() {
      ...
      axios({
        method: "post",
        url: "http://10.6.1.179:40018/mpd/ylxjb/luruf/deleteymgysmc",
        params: {
          gysmc: this.gysmc
        },
      }).then((res) => {
    ...
    deletewymdcwhxxy(id) {
      ...
      axios({
        method: "post",
        url: "http://10.6.1.179:40018/mpd/ylxjb/luruf/deleteymdcwhxxy",
        params: {
          id: id
        },
      }).then((res) => {
