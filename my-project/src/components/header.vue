<template>
  <div class="lw_header">
    <div class="lw_header-1">
      ...
      <div class="text">性价比冶金价值评估模型</div>
...
export default {
  ...
  data() {
    return {
      ...
      chn: "",
      photoURL: "",
      name: ["矿粉测算", "焦炭测算", "无烟煤测算", "烟煤测算", "采购决策"],
      cur: 0,
    };
  },
  methods: {
    ...
    select(text) {
      ...
      this.$axios('http://so.newsteel.ai:8888/shiro/web/authc/isPermitted?permission=sinter:ylcs:view&token=' + token).then((res) => {
        res
      })
    },
    getQueryString(name) {
    ...
    turnsecound() {
      switch (this.cur) {
        ...
        case 3:
          ...
          break;
        case 4:
          this.$router.push({path: "/jfdcwh", query: {}})
      }
    },
  },
  created() {
