  ...
  <div>
    ...
    <div class="fh">
      ...
      <div class="ery">
        ...
        <div class="qhbb2" style="border-radius: 0.2vw">
          <div class="qhbb4" style="font-size: 0.8vw" @click="insertjfwymxxl()">
            配比维护
          </div>
        </div>
        <div class="qhbb2" style="border-radius: 0.2vw">
    ...
    <div class="tablebackground">
      ...
      <div class="thres" style="width: 98.9vw;background-color: #1E2D42">
        <div>
          <span class="texts"> 焦粉名称 </span>
        </div>
        ...
        <div class="xbutt" @click="mxgils()">明细添加</div>
      </div>
      <table class="table">
        <thead>
        <tr>
        ...
        <tbody style="height: 32.3vh">
        <tr v-for="(item, index) in jftablescilma" :key="index">
    ...
    <div class="fh" style="margin-right: 0.5vw">
      <div class="xdxx" style="width: 8vw">数据列表</div>
      <div class="xbutt" @click="quanxuanbut()">全选/取消</div>
    </div>
    <!-- 第六行 table -->
    <div class="tablebackground" style="height: 37.5vh">
      <table class="table">
        <thead>
        <tr>
          <td style="width: 4vw">序号</td>
    ...
    <div v-show="redrscbs" class="turnaongn">
      <div class="whcuia" style="height: 23vh">
        <div class="firgui">
          <div>固体燃耗</div>
          <div style="font-size: 1.5vw" @click="redrscbs=false">×</div>
        </div>
        <div class="firgui" style="width: 27vw;margin-left: 1.5vw">
          <div class="outils" style="width: 13vw">
            <sp>无烟煤固体燃耗</sp>
            <input type="text" class="inputs" v-model="wymrh"/>
          </div>
          <div class="outils" style="width: 12vw">
            <sp>焦粉固体燃耗</sp>
            <input type="text" class="inputs" v-model="jfrh"/>
          </div>
...
export default {
  ...
  methods: {
    rtedbcs() {
      this.$router.push({path: "/dbcs", query: {}});
    },
    //添加焦粉和无烟煤信息
    insertjfwymxxl() {
      if (this.turnbuts==""){
      ...
      axios({
        ...
        this.ourbbh = res.data;
        this.$router.push({name: "dbcs", query: {ourbbha: this.ourbbh}});
      });
    },
    //提交更改焦粉表格信息
    updatejcbghyl() {
    ...
    quedingupdate() {
      ...
      axios({
        method: "post",
        url: "http://10.6.1.179:40018/mpd/ylxjb/luruf/updategtdh",
        params: {
          wymrh: this.wymrh,
          jfrh: this.jfrh,
        },
    ...
    fhtursec() {
      ...
      axios({
        method: "post",
        url: "http://10.6.1.179:40018/mpd/ylxjb/luruf/groupate",
        params: {
          idrs: JSON.stringify(this.turnbuts),
        },
      }).then((res) => {
