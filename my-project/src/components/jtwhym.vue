<template>
  <div>
    ...
    <div class="fh">
      ...
      <div class="ery">
        <div class="qhbb2" style="border-radius: 0.2vw; margin-left: 1vw">
          <div class="qhbb4" style="font-size: 0.8vw" @click="jcwhbut()">
            基础维护
          </div>
        </div>
    ...
    <div class="thres">
      ...
      <div class="xbutt" @click="htjgtjtj()">价格修改</div>
      <div>
        <span class="texts" style="margin-left: 1.5vw"> 基准值: </span>
        <input type="text" class="inputs" v-model="jzzxg"/>
      </div>
      <div class="xbutt" @click="updatejzz()">修改</div>
    ...
    <div class="thres">
      <div>
        <span class="texts"> 燃料类型: </span>
        <select
          class="inputs"
          style="border: none; width: 6.2vw"
          v-model="wlmc"
        >
          <option v-for="(item, index) in selectbuts.rllx" :key="index">
            {{ item }}
          </option>
        </select>
      </div>
      <div style="margin-left: 1.5vw">
        <span class="texts"> 燃料名称: </span>
        <select
          class="inputs"
          style="border: none; width: 6.2vw"
          v-model="gysmc"
        >
          <option v-for="(item, index) in selectbuts.rlmc" :key="index">
            {{ item }}
          </option>
        </select>
      </div>
      ...
      <div>
        <span class="texts" style="margin-left: 1.5vw"> 付款方式: </span>
        <input type="text" class="inputs" v-model="fkfs"/>
      </div>
      <div>
        <span class="texts" style="margin-left: 1.5vw"> 结算水分: </span>
        <input type="text" class="inputs" v-model="jssf"/>
      </div>
      <div>
        <span class="texts" style="margin-left: 1.5vw"> 结算粉末: </span>
        <input type="text" class="inputs" v-model="jsfm"/>
      </div>
      <div>
        <span class="texts" style="margin-left: 1.5vw"> 归属地: </span>
        <select
          class="inputs"
          style="border: none; width: 6.2vw"
          v-model="gsdgls"
        >
          <option v-for="(item, index) in bugralst" :key="index">
            {{ item }}
          </option>
        </select>
      </div>
    ...
    <div class="tablebackground">
      <table class="table">
        ...
        <tr v-for="(item, index) in tablelists" :key="index">
          ...
          <td style="width: 20vw;text-align: left">{{ item.distributorName }}</td>
          <td style="text-align: left;padding-left: 1vw">{{ item.gsdgil }}</td>
          <td style="text-align: right">{{ item.contractPrice }}</td>
          <td style="text-align: right">{{ item.monthAmount }}</td>
          <td style="text-align: left;padding-left: 1vw">{{ item.sterilise }}</td>
          <td style="text-align: right">{{ item.jssf }}</td>
          <td style="text-align: right">{{ item.jsfm }}</td>
          <td>
            <div>
                <span
          ...
          <td @click="jizhuxxwh(item.id)">
            <span v-if="jzxxwhan == item.id">
              <img src="../assets/资源 67@2x.png" style="width: 1.5vw; height: 1.8vh">
            </span>
            <span v-else>
              <img src="../assets/资源 66@2x.png" style="width: 1.5vw; height: 1.8vh">
            </span>
          </td>
          <td @click="deletewymdcwhxxy(item.id)">
    ...
    <div class="xiugai" v-if="updateshow == true">
      <div class="xiugaiyemian">
        ...
        <div class="tfh">
          <div>
            <span class="texts"> 合同价格 </span>
            <input type="text" class="inputs" v-model="htjg"/>
          </div>
          <div>
            <span class="texts" style="margin-left: 1.9vw"> 月计划量 </span>
            <input type="text" class="inputs" v-model="yjhl"/>
          </div>
        </div>
        <!-- 第三行 -->
        <div class="tfh">
          <div>
            <span class="texts"> 付款方式 </span>
            <input type="text" class="inputs" v-model="fkfs"/>
          </div>
          <div>
            <span class="texts" style="margin-left: 1.9vw"> 结算水分 </span>
            <input type="text" class="inputs" v-model="jssf"/>
          </div>
        </div>
        <!-- 第四行 -->
        <div class="tfh">
          <div>
            <span class="texts"> 结算粉末 </span>
            <input type="text" class="inputs" v-model="jsfm"/>
          </div>
          <div>
            <span class="texts" style="margin-left: 1.9vw"> 归属地 </span>
            <select
              class="inputs"
              style="border: none; width: 6.2vw"
              v-model="gsdgls"
...
export default {
  ...
  methods: {
    ...
    deletewymdcwhxxy(id) {
      ...
      axios({
        method: "post",
        url: "http://10.6.1.179:40018/mpd/ylxjb/luruf/deletewymdcjc",
        params: {
          id: id
        },
      }).then((res) => {
...
,
    ...
    selectattributevalue() {
      axios({
        ...
        this.wlmc = this.selectbuts.rllx[0];
        this.gysmc = this.selectbuts.rlmc[0];
      });
    },
    //信息添加至表格中
    insertallofluruf() {
      ...
      axios({
        method: "post",
        url: "http://10.6.1.179:40018/mpd/ylxjb/luruf/insertjcwh",
        params: {
          material_name: this.wlmc,
    ...
    selectofluruf() {
      axios({
        method: "post",
        url: "http://10.6.1.179:40018/mpd/ylxjb/luruf/selectjcwh",
        params: {},
      }).then((res) => {
        this.tablelists = res.data;
        console.log(this.tablelists, 565656)
      });
    },
    //切换按钮
    rebuts(id) {
    ...
    //修改信息
    updatecontext(
      wlmc, gysmc, htjg, yjhl, fkfs, jssf, jsfm, id, gsdgil
    ) {
      this.wlmc = wlmc;
      this.gysmc = gysmc;
      this.htjg = htjg;
    ...
    quedingupdate() {
      ...
      axios({
        method: "post",
        url: "http://10.6.1.179:40018/mpd/ylxjb/luruf/updatejcwhxs",
        params: {
          wlmc: this.wlmc,
          gysmc: this.gysmc,
          htjg: this.htjg,
    ...
    fhtursec() {
      this.$router.push({path: "/jtfirst", query: {}});
    },
    xpwhtur() {
    ...
    htjgtjtj() {
      ...
      if (this.htjgzj != "") {
        axios({
          method: "post",
          url: "http://10.6.1.179:40018/mpd/ylxjb/luruf/updatejcmonthprice",
          params: {
      ...
      if (this.htjgjs != "") {
        axios({
          method: "post",
          url: "http://10.6.1.179:40018/mpd/ylxjb/luruf/updatedatejctj",
          params: {
            tabli: JSON.stringify(this.tablelists),
            htjgjs: this.htjgjs,
          },
    ...
    bzwhxxy() {
      //标准维护弹窗
      this.$router.push({path: "/jtdcwhxxy", query: {}})
    },
    // 标准信息查询
    bzxxcx() {
      axios({
