...
<!--      </div>-->
    ...
    >
      <table class="table">
        ...
        <tr>
          ...
          <td @click="list_sort('mt')" :class="biaotbs=='mt'?'ssors':''" style="cursor: pointer;">KO</td>
          <td @click="list_sort('k')" :class="biaotbs=='k'?'ssors':''" style="cursor: pointer;">NaO</td>
          <td @click="list_sort('na')" :class="biaotbs=='na'?'ssors':''" style="cursor: pointer;">ZnO</td>
          <td>有效热量</td>
          <td>出厂价格</td>
          <td>运费价格</td>
          <td>到厂价格</td>
        ...
        <tr v-for="(item, index) in dcbg" :key="index">
          <td>{{ index + 1 }}</td>
          <td style="width:10vw">{{ item.distributorName }}</td>
          <td>{{ ((item.cgl / murscglgil) * 100).toFixed(1) + '%' }}</td>
          <td>{{ item.cgl }}</td>
          <td>{{ ((item.dxl * 1) * 100).toFixed(2) + '%' }}</td>
          <td  :style=" tatlesonment[0].ad.split('-')[1]*1 < item.ad ||
          item.ad< (tatlesonment[0]).ad.split('-')[0]*1?'color:red':''">{{ item.ad }}</td>
          <td  :style=" tatlesonment[0].stD.split('-')[1]*1 < item.stD ||
          item.stD< (tatlesonment[0]).stD.split('-')[0]*1?'color:red':''">{{ item.stD }}</td>
          <td  :style=" tatlesonment[0].qgrD.split('-')[1]*1 < item.qgrD ||
          item.qgrD< (tatlesonment[0]).qgrD.split('-')[0]*1?'color:red':''">{{ item.qgrD }}</td>
          <td  :style=" tatlesonment[0].vdaf.split('-')[1]*1 < item.vdaf ||
          item.vdaf< (tatlesonment[0]).vdaf.split('-')[0]*1?'color:red':''">{{ item.vdaf }}</td>
          <td :style=" tatlesonment[0].fcd.split('-')[1]*1 < item.fcd ||
          item.fcd< (tatlesonment[0]).fcd.split('-')[0]*1?'color:red':''">{{ item.fcd }}</td>
          <td :style=" tatlesonment[0].mt.split('-')[1]*1 < item.mt ||
          item.mt< (tatlesonment[0]).mt.split('-')[0]*1?'color:red':''">{{ item.mt }}</td>
          <td :style=" tatlesonment[0].mad.split('-')[1]*1 < item.mad ||
          item.mad< (tatlesonment[0]).mad.split('-')[0]*1?'color:red':''">{{ item.mad }}</td>
          <td :style=" tatlesonment[0].ko.split('-')[1]*1 < item.k ||
          item.k< (tatlesonment[0]).ko.split('-')[0]*1?'color:red':''">{{ item.k }}</td>
          <td :style=" tatlesonment[0].nao.split('-')[1]*1 < item.na ||
          item.na< (tatlesonment[0]).nao.split('-')[0]*1?'color:red':''">{{ item.na }}</td>
          <td :style=" tatlesonment[0].zno.split('-')[1]*1 < item.zn ||
          item.zn< (tatlesonment[0]).zno.split('-')[0]*1?'color:red':''">{{ item.zn }}</td>
          <td>{{ item.yxrl }}</td>
          <td>{{ item.ccjg }}</td>
          <td>{{ item.yfjg }}</td>
          <td>{{ item.dcjg }}</td>
          <td>{{ item.sterilise }}</td>
          <td>{{ item.jssf }}</td>
          <td>{{ item.bhsjg }}</td>
          <td>{{ item.dwyxrcb }}</td>
          <td @click="gtilgrals(item.distributorName,item.sterilise,item.ccjg,
          item.cgl,item.jssf,item.yfjg,item.dcjg,item.yxrl,item.ad,item.stD,
          item.vdaf,item.fcd,item.qgrD)">
    ...
    <div class="xjbpxli">
      <div class="xjbpxlisize">
        <span class="posi" style="margin-top: 1vh">采购兑现率</span>
      </div>
      ...
      <div class="xjbpxlisize2 jsc">
        ...
        <div>
          <select
            ...
            v-model="ssexllhxn"
          >
            <option v-for="(item, index) in lhxnils" :key="index">
              {{ item }}
            </option>
          </select>
        </div>
      ...
      <div class="tcstyle">
        ...
        <div class="zfkyas">
          ...
          <div class="fkyss">
            ...
            <div class="neirongtc">{{ this.jjbs }}</div>
          </div>
          <div class="fkyss">
            <div class="biaotifk">运费价格</div>
            <div class="neirongtc">{{ this.mjbs }}</div>
          </div>
          <div class="fkyss">
            <div class="biaotifk">到厂价格</div>
            <div class="neirongtc">{{ this.mjcbs }}</div>
          </div>
          <div class="fkyss">
            <div class="biaotifk">有效热量</div>
            <div class="neirongtc">{{ this.ads }}</div>
          </div>
        </div>
        <!-- 纵向两个方块 -->
        <div class="zfks">
              ...
              <div class="leftxiugaian">
                <div class="leftsecound">
                  ...
                  <div class="leftsectwo" style="margin-right: 0.5vw">
                    <span class="yuelejsj">到厂价格</span>
                    <span class="yuelejsj" style="color: orange">{{
                        this.mjcbs
                      }}</span>
                    <span class="yuelejsj">元</span>
                  </div>
                </div>
...
export default {
  data() {
    return {
      ...
      qursketgor: "",
      //理化性能
      lhxnils: ["可磨性", "粘结性", "燃烧性", "反应性", "流动性", "喷流性", "灰熔点", "着火点"],
      ssexllhxn: "",
      //理化性能图数据
      lhxnecha: [],
    };
  ...
  methods: {
    selectjtdcxxwhwymfw() {
      axios({
        method: "post",
        url: "http://10.6.1.179:40018/mpd/ylxjb/luruf/selectjtdcxxwhwymfw",
        params: {},
      }).then((res) => {
    ...
    csbbxi() {
      ...
      axios({
        method: "post",
        url: "http://10.6.1.179:40018/mpd/ylxjb/luruf/selecttocopycopywym",
        params: {
          bbh: this.banbenhao,
        },
      }).then((res) => {
        ...
        for (let i = 0; i < res.data.length; i++) {
          res.data[i].distributorName = res.data[i].distributorName
            .replace("有限责任公司", "")
            .replace("矿业", "")
            .replace("公用设施", "")
            .replace("钢铁", "")
    ...
    taml() {
      ...
      axios({
        method: "post",
        url: "http://10.6.1.179:40018/mpd/ylxjb/luruf/selecttocopycopywymqj",
        params: {
          bbh: this.qursketgor,
          tml: this.value1,
          tal: this.value2,
        ...
        this.btbugrs = [];
        for (let i = 0; i < res.data.length; i++) {
          res.data[i].distributorName = res.data[i].distributorName.replace("有限责任公司", "")
            .replace("矿业", "")
            .replace("公用设施", "")
            .replace("钢铁", "")
            .replace("国际贸易", "")
    ...
    lhxntulit() {
      axios({
        method: "post",
        url: "http://10.6.1.179:40018/mpd/ylxjb/luruf/selectlhxnlit",
        params: {
          wz: this.ssexllhxn,
        },
      }).then((res) => {
        this.lhxnecha = res.data;
        var ag = []
        var va = []
        for (let i = 0; i < this.lhxnecha.length; i++) {
          ag.push(this.lhxnecha[i].gysmc.replace("有限公司", "")
            .replace("有限责任公司", "")
            .replace("矿业", "")
            .replace("公用设施", "")
            .replace("钢铁", "")
    ...
    svered() {
      axios({
        method: "post",
        url: "http://10.6.1.179:40018/mpd/ylxjb/luruf/selectbehedwym",
        params: {},
      }).then((res) => {
      ...
      switch (this.ssexl) {
        case "Ad":
          for (let i = 0; i < this.dcbg.length; i++) {
            biathed.push({
              cma: this.dcbg[i].distributorName.substring(0,9),
              cma1: this.dcbg[i].ad
            })
          }
          break;
    ...
    draw() {
      ...
      for (let i = 0; i < titme.length; i++) {
        ...
        ticgial.push(titme[i].name1.substring(0,9))
      }
      ...
      let option = {
        ...
        radar: {
          indicator: [
            ...
            {name: "Vdaf" + "(" + tats[2] + ")", max: 15},
            {name: "FCd" + "(" + tats[3] + ")", max: 90},
            {name: "Qgr,d" + "(" + tats[4] + ")", max: 8500},
          ],
          name: {
            textStyle: {color: "#fff"},
          },
    ...
    dexls() {
      ...
      for (let i = this.tmilkestial.length - 1; i >= 0; i--) {
        ...
        bar.push(this.tmilkestial[i].cme1.substring(0,9));
        console.log(bar)
      }
    ...
    selectzztclas() {
      ...
      axios({
        method: "get",
        url: "http://10.6.1.179:40018/mpd/ylxjb/luruf/selectsixmtachrwym",
        params: {
    ...
    //历史弹窗
    gtilgrals(distributorName, sterilise, ccjg, cgl, jssf,
              yfjg, dcjg, yxrl, ad, stD, vdaf, fcd, qgrD) {
      this.histtorytcs = true;
