  ...
  methods: {
    ...
    selectztbbxis(gysmc) {
      (this.ztbbxxsys = []),
        axios({
          method: "post",
          url: "http://localhost:40013/mpd/ylxjb/luruf/selecttcymsj",
          params: {
            bbh: this.banbenhao,
            gysmc: gysmc,
          },
          ...
          console.log(this.ztbbxxsys, 9999999999);
        });
    },
    //折叠弹窗 柱状图信息
    selectzztclas() {
      axios({
        method: "post",
        url: "http://localhost:40013/mpd/ylxjb/luruf/selecttypeofzut",
        params: {
          gysmc: this.zzgysmc,
        ...
        let data = [];
        let timeID = [];
        for (let i in this.zztdz) {
          data.push(this.zztdz[i]);
          timeID.push(i);
        }
        console.log(data, timeID, 123);

        console.log(this.zztdz, 99595864841231658);
      });
    },
    // 打开弹窗
    opentancs(
